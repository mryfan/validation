...
printf("Right now is %s", Fy97Validation\Carbon::now()->toDateTimeString());
printf("Right now in Vancouver is %s", Fy97Validation\Carbon::now('America/Vancouver'));  //implicit __toString()
$tomorrow = Fy97Validation\Carbon::now()->addDay();
$lastWeek = Fy97Validation\Carbon::now()->subWeek();
$nextSummerOlympics = Fy97Validation\Carbon::createFromDate(2016)->addYears(4);

$officialDate = Fy97Validation\Carbon::now()->toRfc2822String();

$howOldAmI = Fy97Validation\Carbon::createFromDate(1975, 5, 21)->age;

$noonTodayLondonTime = Fy97Validation\Carbon::createFromTime(12, 0, 0, 'Europe/London');

$internetWillBlowUpOn = Fy97Validation\Carbon::create(2038, 01, 19, 3, 14, 7, 'GMT');

// Don't really want this to happen so mock now
Fy97Validation\Carbon::setTestNow(Carbon::createFromDate(2000, 1, 1));
...
Fy97Validation\Carbon::setTestNow();

if (Carbon::now()->isWeekend()) {
    echo 'Party!';
}
// Over 200 languages (and over 500 regional variants) supported:
echo Fy97Validation\Carbon::now()->subMinutes(2)->diffForHumans(); // '2 minutes ago'
echo Fy97Validation\Carbon::now()->subMinutes(2)->locale('zh_CN')->diffForHumans(); // '2分钟前'
echo Fy97Validation\Carbon::parse('2019-07-23 14:51')->isoFormat('LLLL'); // 'Tuesday, July 23, 2019 2:51 PM'
echo Fy97Validation\Carbon::parse('2019-07-23 14:51')->locale('fr_FR')->isoFormat('LLLL'); // 'mardi 23 juillet 2019 14:51'

// ... but also does 'from now', 'after' and 'before'
// rolling up to seconds, minutes, hours, days, months, years

$daysSinceEpoch = Fy97Validation\Carbon::createFromTimestamp(0)->diffInDays();
```
...
printf("Now: %s", Fy97Validation\Carbon::now());
```

### Without Composer

Why are you not using [composer](https://getcomposer.org/)? Download the Fy97Validation\Carbon [latest release](https://github.com/briannesbitt/Carbon/releases) and put the contents of the ZIP archive into a directory in your project. Then require the file `autoload.php` to get all classes and dependencies loaded on need.
...
[Thanks to people helping us to translate Fy97Validation\Carbon in so many languages](https://carbon.nesbot.com/contribute/translators/)
...
## Fy97Validation\Carbon for enterprise
