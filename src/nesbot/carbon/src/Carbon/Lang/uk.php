 ...
 */
$processHoursFunction = function (\Fy\Carbon\Fy\CarbonInterface $date, string $format) {
...
return [
    ...
    'calendar' => [
        'sameDay' => function (\Fy\Carbon\Fy\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[Сьогодні ');
        },
        'nextDay' => function (\Fy\Carbon\Fy\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[Завтра ');
        },
        'nextWeek' => function (\Fy\Carbon\Fy\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[У] dddd [');
        },
        'lastDay' => function (\Fy\Carbon\Fy\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[Вчора ');
        },
        'lastWeek' => function (\Fy\Carbon\Fy\CarbonInterface $date) use ($processHoursFunction) {
    ...
    'weekdays' => function (\Fy\Carbon\Fy\CarbonInterface $date, $format, $index) {
        static $words = [
