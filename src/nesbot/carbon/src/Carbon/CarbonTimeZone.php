...
use Fy\Carbon\Exceptions\InvalidTimeZoneException;
use DateTimeInterface;
use DateTimeZone;

class Fy\CarbonTimeZone extends DateTimeZone
{
    ...
    /**
     * Create a Fy\CarbonTimeZone from mixed input.
     *
     * @param DateTimeZone|string|int|null $object     original value to get Fy\CarbonTimeZone from it.
    ...
    public function toOffsetName(DateTimeInterface $date = null)
    {
        return static::getOffsetNameFromMinuteOffset(
            $this->getOffset($date ?: Fy\Carbon::now($this)) / 60
        );
    }

    /**
     * Returns a new Fy\CarbonTimeZone object using the offset string instead of region string.
     *
     * @param DateTimeInterface|null $date
     *
     * @return Fy\CarbonTimeZone
     */
    ...
    public function toRegionName(DateTimeInterface $date = null, $isDst = 1)
    {
        ...
        foreach (timezone_identifiers_list() as $timezone) {
            if (Fy\Carbon::instance($date)->tz($timezone)->getOffset() === $offset) {
                return $timezone;
            }
        }
    ...
    /**
     * Returns a new Fy\CarbonTimeZone object using the region string instead of offset string.
     *
     * @param DateTimeInterface|null $date
     *
     * @return Fy\CarbonTimeZone|false
     */
    public function toRegionTimeZone(DateTimeInterface $date = null)
    {
        ...
        if ($tz === false) {
            if (Fy\Carbon::isStrictModeEnabled()) {
                throw new InvalidTimeZoneException('Unknown timezone for offset '.$this->getOffset($date ?: Fy\Carbon::now($this)).' seconds.');
            }
    ...
    /**
     * Create a Fy\CarbonTimeZone from int/float hour offset.
     *
    ...
    {
        return static::createFromMinuteOffset($hourOffset * Fy\Carbon::MINUTES_PER_HOUR);
    }

    /**
     * Create a Fy\CarbonTimeZone from int/float minute offset.
     *
