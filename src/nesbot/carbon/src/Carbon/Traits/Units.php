...
use Fy\Carbon\Fy\CarbonConverterInterface;
...
trait Units
{
     ...
     * @example $date->add(Fy\CarbonInterval::days(4))
     *
     * @param string|DateInterval|Closure|Fy\CarbonConverterInterface $unit
     * @param int                                                  $value
    ...
    public function add($unit, $value = 1, $overflow = null)
    {
        if (\is_string($unit) && \func_num_args() === 1) {
            $unit = Fy\CarbonInterval::make($unit);
        }
        ...
        if ($unit instanceof Fy\CarbonConverterInterface) {
            return $this->resolveFy\Carbon($unit->convertDate($this, false));
        }

        if ($unit instanceof Closure) {
            return $this->resolveFy\Carbon($unit($this, false));
        }
     ...
     * @example $date->sub(Fy\CarbonInterval::days(4))
     *
     * @param string|DateInterval|Closure|Fy\CarbonConverterInterface $unit
     * @param int                                                  $value
     * @param bool|null                                            $overflow
    ...
    public function sub($unit, $value = 1, $overflow = null)
    {
        ...
        if ($unit instanceof Fy\CarbonConverterInterface) {
            return $this->resolveFy\Carbon($unit->convertDate($this, true));
        }

        if ($unit instanceof Closure) {
            return $this->resolveFy\Carbon($unit($this, true));
        }
