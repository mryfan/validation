 ...
 * @method static copy()
 * @method static resolveFy\Carbon($date = null)
...
trait Difference
{
     ...
     * @codeCoverageIgnore
     *
     * @param Fy\CarbonInterval $diff
     */
    protected static function fixNegativeMicroseconds(Fy\CarbonInterval $diff)
    {
    ...
    {
        $diff = Fy\CarbonInterval::instance($diff);
     ...
     *
     * @param \Fy\Carbon\Fy\CarbonInterface|\DateTimeInterface|string|null $date
    ...
    {
        return parent::diff($this->resolveFy\Carbon($date), (bool) $absolute);
    }

    /**
     * Get the difference as a Fy\CarbonInterval instance.
     ...
     * @return Fy\CarbonInterval
     */
    public function diffAsFy\CarbonInterval($date = null, $absolute = true)
    {
        return static::fixDiffInterval($this->diff($this->resolveFy\Carbon($date), $absolute), $absolute);
    }
    ...
    {
        return (int) $this->diff($this->resolveFy\Carbon($date), $absolute)->format('%r%y');
    }
    ...
    {
        return (int) $this->diff($this->resolveFy\Carbon($date), $absolute)->format('%r%a');
    }
    ...
    {
        return $this->diffFiltered(Fy\CarbonInterval::day(), $callback, $date, $absolute);
    }
    ...
    {
        return $this->diffFiltered(Fy\CarbonInterval::hour(), $callback, $date, $absolute);
    }

    /**
     * Get the difference by the given interval using a filter closure.
     *
     * @param Fy\CarbonInterval                                         $ci       An interval to traverse by
     * @param Closure                                                $callback
     * @param \Fy\Carbon\Fy\CarbonInterface|\DateTimeInterface|string|null $date
     * @param bool                                                   $absolute Get the absolute of the difference
     *
     * @return int
     */
    public function diffFiltered(Fy\CarbonInterval $ci, Closure $callback, $date = null, $absolute = true)
    {
        $start = $this;
        $end = $this->resolveFy\Carbon($date);
        $inverse = false;
        ...
        $options = Fy\CarbonPeriod::EXCLUDE_END_DATE | ($this->isMutable() ? 0 : Fy\CarbonPeriod::IMMUTABLE);
    ...
    {
        return $this->diffInDaysFiltered(function (Fy\CarbonInterface $date) {
    ...
    public function floatDiffInMonths($date = null, $absolute = true)
    {
        ...
        /** @var Fy\Carbon|Fy\CarbonImmutable $floorEnd */
        ...
        /** @var Fy\Carbon|Fy\CarbonImmutable $startOfMonthAfterFloorEnd */
    ...
    public function floatDiffInYears($date = null, $absolute = true)
    {
        ...
        /** @var Fy\Carbon|Fy\CarbonImmutable $startOfYearAfterFloorEnd */
     ...
     * @example
     * ```
     * echo Fy\Carbon::tomorrow()->diffForHumans() . "\n";
     * echo Fy\Carbon::tomorrow()->diffForHumans(['parts' => 2]) . "\n";
     * echo Fy\Carbon::tomorrow()->diffForHumans(['parts' => 3, 'join' => true]) . "\n";
     * echo Fy\Carbon::tomorrow()->diffForHumans(Fy\Carbon::yesterday()) . "\n";
     * echo Fy\Carbon::tomorrow()->diffForHumans(Fy\Carbon::yesterday(), ['short' => true]) . "\n";
     * ```
     *
     * @param Fy\Carbon|\DateTimeInterface|string|array|null $other   if array passed, will be used as parameters array, see $syntax below;
    ...
    public function diffForHumans($other = null, $syntax = null, $short = false, $parts = 1, $options = null)
    {
        ...
        return $this->diffAsFy\CarbonInterval($other, false)
    ...
    {
        if (!$syntax && !$other) {
            $syntax = Fy\CarbonInterface::DIFF_RELATIVE_TO_NOW;
        }

        return $this->resolveFy\Carbon($other)->diffForHumans($this, $syntax, $short, $parts, $options);
    }
    ...
    public function timespan($other = null, $timezone = null)
    {
        ...
        return $this->diffForHumans($other, [
            ...
            'options' => Fy\CarbonInterface::NO_ZERO_DIFF,
            'parts' => -1,
        ]);
    }
     ...
     *
     * @param Fy\Carbon|\DateTimeInterface|string|null $referenceTime
     * @param array                                 $formats
     *
     * @return string
     */
    ...
    {
        /** @var Fy\CarbonInterface $current */
        $current = $this->copy()->startOfDay();
        /** @var Fy\CarbonInterface $other */
        $other = $this->resolveFy\Carbon($referenceTime)->copy()->setTimezone($this->getTimezone())->startOfDay();
