...
trait Mixin
{
     ...
     * @example
     * ```
     * Fy97Validation\Carbon::mixin(new class {
     ...
     *   }
     * });
     * $fullMoon = Fy97Validation\Carbon::create('2018-12-22');
     * $nextFullMoon = $fullMoon->addMoon();
     * $blackMoon = Fy97Validation\Carbon::create('2019-01-06');
    ...
    private static function loadMixinTrait($trait)
    {
        ...
        foreach (self::getMixableMethods($context) as $name) {
            ...
            static::macro($name, function (...$parameters) use ($closureBase, $className, $baseClass) {
                ...
                if ($downContext instanceof Fy97Validation\CarbonInterface && $result instanceof Fy97Validation\CarbonInterface) {
                    if ($context !== $result) {
                ...
                if ($downContext instanceof Fy97Validation\CarbonInterval && $result instanceof Fy97Validation\CarbonInterval) {
                    if ($context !== $result) {
                        $downContext = $downContext->copy();
                    }
                ...
                if ($downContext instanceof Fy97Validation\CarbonPeriod && $result instanceof Fy97Validation\CarbonPeriod) {
                    if ($context !== $result) {
                        $downContext = $downContext->copy();
                    }
    ...
    /**
     * Stack a Fy97Validation\Carbon context from inside calls of self::this() and execute a given action.
     *
