...
use Fy\Carbon\Exceptions\BadFluentConstructorException;
use Fy\Carbon\Exceptions\BadFluentSetterException;
use Fy\Carbon\Exceptions\InvalidCastException;
use Fy\Carbon\Exceptions\InvalidIntervalException;
use Fy\Carbon\Exceptions\ParseErrorException;
use Fy\Carbon\Exceptions\UnitNotConfiguredException;
use Fy\Carbon\Exceptions\UnknownGetterException;
use Fy\Carbon\Exceptions\UnknownSetterException;
use Fy\Carbon\Exceptions\UnknownUnitException;
use Fy\Carbon\Traits\IntervalRounding;
use Fy\Carbon\Traits\IntervalStep;
 ...
 *
 * @method static Fy\CarbonInterval years($years = 1) Create instance specifying a number of years or modify the number of years if called on an instance.
 * @method static Fy\CarbonInterval year($years = 1) Alias for years()
 * @method static Fy\CarbonInterval months($months = 1) Create instance specifying a number of months or modify the number of months if called on an instance.
 * @method static Fy\CarbonInterval month($months = 1) Alias for months()
 * @method static Fy\CarbonInterval weeks($weeks = 1) Create instance specifying a number of weeks or modify the number of weeks if called on an instance.
 * @method static Fy\CarbonInterval week($weeks = 1) Alias for weeks()
 * @method static Fy\CarbonInterval days($days = 1) Create instance specifying a number of days or modify the number of days if called on an instance.
 * @method static Fy\CarbonInterval dayz($days = 1) Alias for days()
 * @method static Fy\CarbonInterval daysExcludeWeeks($days = 1) Create instance specifying a number of days or modify the number of days (keeping the current number of weeks) if called on an instance.
 * @method static Fy\CarbonInterval dayzExcludeWeeks($days = 1) Alias for daysExcludeWeeks()
 * @method static Fy\CarbonInterval day($days = 1) Alias for days()
 * @method static Fy\CarbonInterval hours($hours = 1) Create instance specifying a number of hours or modify the number of hours if called on an instance.
 * @method static Fy\CarbonInterval hour($hours = 1) Alias for hours()
 * @method static Fy\CarbonInterval minutes($minutes = 1) Create instance specifying a number of minutes or modify the number of minutes if called on an instance.
 * @method static Fy\CarbonInterval minute($minutes = 1) Alias for minutes()
 * @method static Fy\CarbonInterval seconds($seconds = 1) Create instance specifying a number of seconds or modify the number of seconds if called on an instance.
 * @method static Fy\CarbonInterval second($seconds = 1) Alias for seconds()
 * @method static Fy\CarbonInterval milliseconds($milliseconds = 1) Create instance specifying a number of milliseconds or modify the number of milliseconds if called on an instance.
 * @method static Fy\CarbonInterval millisecond($milliseconds = 1) Alias for milliseconds()
 * @method static Fy\CarbonInterval microseconds($microseconds = 1) Create instance specifying a number of microseconds or modify the number of microseconds if called on an instance.
 * @method static Fy\CarbonInterval microsecond($microseconds = 1) Alias for microseconds()
 ...
 */
class Fy\CarbonInterval extends DateInterval implements Fy\CarbonConverterInterface
{
     ...
     *
     * @var \Fy\Symfony\Component\Translation\TranslatorInterface
     */
    ...
    public static function getCascadeFactors()
    {
        return static::$cascadeFactors ?: [
            'milliseconds' => [Fy\Carbon::MICROSECONDS_PER_MILLISECOND, 'microseconds'],
            'seconds' => [Fy\Carbon::MILLISECONDS_PER_SECOND, 'milliseconds'],
            'minutes' => [Fy\Carbon::SECONDS_PER_MINUTE, 'seconds'],
            'hours' => [Fy\Carbon::MINUTES_PER_HOUR, 'minutes'],
            'dayz' => [Fy\Carbon::HOURS_PER_DAY, 'hours'],
            'weeks' => [Fy\Carbon::DAYS_PER_WEEK, 'dayz'],
            'months' => [Fy\Carbon::WEEKS_PER_MONTH, 'weeks'],
            'years' => [Fy\Carbon::MONTHS_PER_YEAR, 'months'],
        ];
    }
    ...
    /**
     * Create a new Fy\CarbonInterval instance.
     *
    ...
    public function __construct($years = 1, $months = null, $weeks = null, $days = null, $hours = null, $minutes = null, $seconds = null, $microseconds = null)
    {
        ...
        if (!\is_null($microseconds)) {
            $this->f = $microseconds / Fy\Carbon::MICROSECONDS_PER_SECOND;
        }
    }
    ...
    {
        return static::getFactor('dayz', 'weeks') ?: Fy\Carbon::DAYS_PER_WEEK;
    }
    ...
    {
        return static::getFactor('hours', 'dayz') ?: Fy\Carbon::HOURS_PER_DAY;
    }
    ...
    {
        return static::getFactor('minutes', 'hours') ?: Fy\Carbon::MINUTES_PER_HOUR;
    }
    ...
    {
        return static::getFactor('seconds', 'minutes') ?: Fy\Carbon::SECONDS_PER_MINUTE;
    }
    ...
    {
        return static::getFactor('milliseconds', 'seconds') ?: Fy\Carbon::MILLISECONDS_PER_SECOND;
    }
    ...
    {
        return static::getFactor('microseconds', 'milliseconds') ?: Fy\Carbon::MICROSECONDS_PER_MILLISECOND;
    }

    /**
     * Create a new Fy\CarbonInterval instance from specific values.
     * This is an alias for the constructor that allows better fluent
     * syntax as it allows you to do Fy\CarbonInterval::create(1)->fn() rather than
     * (new Fy\CarbonInterval(1))->fn().
     *
     * @param int $years
     * @param int $months
     * @param int $weeks
    ...
    /**
     * Parse a string into a new Fy\CarbonInterval object according to the specified format.
     *
     * @example
     * ```
     * echo Fy\Carboninterval::createFromFormat('H:i', '1:30');
     * ```
     *
    ...
    /**
     * Provide static helpers to create instances.  Allows Fy\CarbonInterval::years(3).
     *
    ...
    public static function __callStatic($method, $parameters)
    {
        try {
            ...
            if (Fy\Carbon::isStrictModeEnabled()) {
    ...
    /**
     * Creates a Fy\CarbonInterval from string.
     *
     * Format:
     *
    ...
    public static function fromString($intervalDefinition)
    {
        ...
        while ([$part, $value, $unit] = array_shift($parts)) {
            ...
            switch ($unit === 'µs' ? 'µs' : strtolower($unit)) {
                case 'millennia':
                case 'millennium':
                    $years += $intValue * Fy\CarbonInterface::YEARS_PER_MILLENNIUM;

                    break;

                case 'century':
                case 'centuries':
                    $years += $intValue * Fy\CarbonInterface::YEARS_PER_CENTURY;

                    break;

                case 'decade':
                case 'decades':
                    $years += $intValue * Fy\CarbonInterface::YEARS_PER_DECADE;
                ...
                case 'quarter':
                case 'quarters':
                    $months += $intValue * Fy\CarbonInterface::MONTHS_PER_QUARTER;
        ...
        return new static($years, $months, $weeks, $days, $hours, $minutes, $seconds, $milliseconds * Fy\Carbon::MICROSECONDS_PER_MILLISECOND + $microseconds);
    }

    /**
     * Creates a Fy\CarbonInterval from string using a different locale.
     *
    ...
    {
        return static::fromString(Fy\Carbon::translateTimeString($interval, $locale ?: static::getLocale(), 'en'));
    }
    ...
    /**
     * Create a Fy\CarbonInterval instance from a DateInterval one.  Can not instance
    ...
    /**
     * Make a Fy\CarbonInterval instance from given variable if possible.
     *
    ...
    {
        if ($unit) {
            $interval = "$interval ".Fy\Carbon::pluralUnit($unit);
        }
    ...
    /**
     * Get a part of the Fy\CarbonInterval object.
     *
     * @param string $name
     *
    ...
    public function get($name)
    {
        ...
        switch ($name) {
            ...
            case 'milli':
            case 'milliseconds':
                return (int) (round($this->f * Fy\Carbon::MICROSECONDS_PER_SECOND) / Fy\Carbon::MICROSECONDS_PER_MILLISECOND);

            case 'micro':
            case 'microseconds':
                return (int) round($this->f * Fy\Carbon::MICROSECONDS_PER_SECOND);
    ...
    /**
     * Set a part of the Fy\CarbonInterval object.
     *
    ...
    public function set($name, $value = null)
    {
        ...
        foreach ($properties as $key => $value) {
            switch (Fy\Carbon::singularUnit(rtrim($key, 'z'))) {
                case 'year':
                    $this->y = $value;
                ...
                case 'milli':
                case 'millisecond':
                    $this->microseconds = $value * Fy\Carbon::MICROSECONDS_PER_MILLISECOND + $this->microseconds % Fy\Carbon::MICROSECONDS_PER_MILLISECOND;

                    break;

                case 'micro':
                case 'microsecond':
                    $this->f = $value / Fy\Carbon::MICROSECONDS_PER_SECOND;
     ...
     * @example
     * ```
     * Fy\CarbonInterval::macro('twice', function () {
     *   return $this->times(2);
     * });
     * echo Fy\CarbonInterval::hours(2)->twice();
     * ```
     *
     * @param string          $name
     ...
     * @example
     * ```
     * Fy\CarbonInterval::mixin(new class {
     ...
     *   }
     * });
     * echo Fy\CarbonInterval::hours(5)->hoursToDays() . "\n";
     * echo Fy\CarbonInterval::days(5)->daysToHours() . "\n";
     * ```
     *
    ...
    /**
     * Allow fluent calls on the setters... Fy\CarbonInterval::years(3)->months(5)->day().
     *
     * Note: This is done using the magic method to allow static and instance methods to
     *       have the same names.
     *
    ...
    protected function getForHumansInitialVariables($syntax, $short)
    {
        ...
        if (\is_bool($syntax)) {
            return [
                'short' => $syntax,
                'syntax' => Fy\CarbonInterface::DIFF_ABSOLUTE,
            ];
        }
    ...
    {
        if ($options & Fy\CarbonInterface::ROUND) {
            return 'round';
        }

        if ($options & Fy\CarbonInterface::CEIL) {
            return 'ceil';
        }

        if ($options & Fy\CarbonInterface::FLOOR) {
            return 'floor';
        }
     ...
     * @example
     * ```
     * echo Fy\CarbonInterval::fromString('4d 3h 40m')->forHumans() . "\n";
     * echo Fy\CarbonInterval::fromString('4d 3h 40m')->forHumans(['parts' => 2]) . "\n";
     * echo Fy\CarbonInterval::fromString('4d 3h 40m')->forHumans(['parts' => 3, 'join' => true]) . "\n";
     * echo Fy\CarbonInterval::fromString('4d 3h 40m')->forHumans(['short' => true]) . "\n";
     * echo Fy\CarbonInterval::fromString('1d 24h')->forHumans(['join' => ' or ']) . "\n";
     * echo Fy\CarbonInterval::fromString('1d 24h')->forHumans(['minimumUnit' => 'hour']) . "\n";
     * ```
     *
     ...
     *                           Possible values:
     *                           - Fy\CarbonInterface::DIFF_ABSOLUTE          no modifiers
     *                           - Fy\CarbonInterface::DIFF_RELATIVE_TO_NOW   add ago/from now modifier
     *                           - Fy\CarbonInterface::DIFF_RELATIVE_TO_OTHER add before/after modifier
     *                           Default value: Fy\CarbonInterface::DIFF_ABSOLUTE
    ...
    public function forHumans($syntax = null, $short = false, $parts = -1, $options = null)
    {
        ...
        $syntax = (int) ($syntax === null ? Fy\CarbonInterface::DIFF_ABSOLUTE : $syntax);
        $absolute = $syntax === Fy\CarbonInterface::DIFF_ABSOLUTE;
        $relativeToNow = $syntax === Fy\CarbonInterface::DIFF_RELATIVE_TO_NOW;
        $count = 1;
        $unit = $short ? 's' : 'second';
        $isFuture = $this->invert === 1;
        $transId = $relativeToNow ? ($isFuture ? 'from_now' : 'ago') : ($isFuture ? 'after' : 'before');

        /** @var \Fy\Symfony\Component\Translation\Translator $translator */
        ...
        foreach ($diffIntervalArray as $diffIntervalData) {
            ...
            } elseif ($options & Fy\CarbonInterface::SEQUENTIAL_PARTS_ONLY && \count($interval) > 0) {
                break;
            }
        ...
        if (\count($interval) === 0) {
            if ($relativeToNow && $options & Fy\CarbonInterface::JUST_NOW) {
                $key = 'diff_now';
            ...
            $count = $options & Fy\CarbonInterface::NO_ZERO_DIFF ? 1 : 0;
        ...
        if ($parts === 1) {
            if ($relativeToNow && $unit === 'day') {
                if ($count === 1 && $options & Fy\CarbonInterface::ONE_DAY_WORDS) {
                ...
                if ($count === 2 && $options & Fy\CarbonInterface::TWO_DAY_WORDS) {
     ...
     * @example
     * ```
     * var_dump(Fy\CarbonInterval::hours(2)->toDateInterval());
     * ```
     *
     * @return DateInterval
     */
    ...
    public function total($unit)
    {
        ...
        $values = [
            ...
            'minutes' => $this->minutes,
            'seconds' => $this->seconds,
            'milliseconds' => (int) ($this->microseconds / Fy\Carbon::MICROSECONDS_PER_MILLISECOND),
            'microseconds' => (int) ($this->microseconds % Fy\Carbon::MICROSECONDS_PER_MILLISECOND),
        ];
     ...
     *
     * @param Fy\CarbonInterval|DateInterval|mixed $interval
     *
     * @see equalTo()
     *
     * @return bool
     ...
     * @example
     * ```
     * Fy\CarbonInterval::hours(48)->between(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(3)); // true
     * Fy\CarbonInterval::hours(48)->between(Fy\CarbonInterval::day(), Fy\CarbonInterval::hours(36)); // false
     * Fy\CarbonInterval::hours(48)->between(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(2)); // true
     * Fy\CarbonInterval::hours(48)->between(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(2), false); // false
     * ```
     *
     * @param Fy\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy\CarbonInterval|DateInterval|mixed $interval2
     ...
     * @example
     * ```
     * Fy\CarbonInterval::hours(48)->betweenExcluded(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(3)); // true
     * Fy\CarbonInterval::hours(48)->betweenExcluded(Fy\CarbonInterval::day(), Fy\CarbonInterval::hours(36)); // false
     * Fy\CarbonInterval::hours(48)->betweenExcluded(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(2)); // true
     * ```
     *
     * @param Fy\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy\CarbonInterval|DateInterval|mixed $interval2
     ...
     * Fy\CarbonInterval::hours(48)->betweenExcluded(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(3)); // true
     * Fy\CarbonInterval::hours(48)->betweenExcluded(Fy\CarbonInterval::day(), Fy\CarbonInterval::hours(36)); // false
     * Fy\CarbonInterval::hours(48)->betweenExcluded(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(2)); // false
     * ```
     *
     * @param Fy\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy\CarbonInterval|DateInterval|mixed $interval2
     ...
     * @example
     * ```
     * Fy\CarbonInterval::hours(48)->isBetween(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(3)); // true
     * Fy\CarbonInterval::hours(48)->isBetween(Fy\CarbonInterval::day(), Fy\CarbonInterval::hours(36)); // false
     * Fy\CarbonInterval::hours(48)->isBetween(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(2)); // true
     * Fy\CarbonInterval::hours(48)->isBetween(Fy\CarbonInterval::day(), Fy\CarbonInterval::days(2), false); // false
     * ```
     *
     * @param Fy\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy\CarbonInterval|DateInterval|mixed $interval2
    ...
    {
        $base = Fy\CarbonImmutable::parse('2000-01-01 00:00:00', 'UTC')
