...
use Fy97Validation\Carbon\Exceptions\BadFluentConstructorException;
use Fy97Validation\Carbon\Exceptions\BadFluentSetterException;
use Fy97Validation\Carbon\Exceptions\InvalidCastException;
use Fy97Validation\Carbon\Exceptions\InvalidIntervalException;
use Fy97Validation\Carbon\Exceptions\OutOfRangeException;
use Fy97Validation\Carbon\Exceptions\ParseErrorException;
use Fy97Validation\Carbon\Exceptions\UnitNotConfiguredException;
use Fy97Validation\Carbon\Exceptions\UnknownGetterException;
use Fy97Validation\Carbon\Exceptions\UnknownSetterException;
use Fy97Validation\Carbon\Exceptions\UnknownUnitException;
use Fy97Validation\Carbon\Traits\IntervalRounding;
use Fy97Validation\Carbon\Traits\IntervalStep;
use Fy97Validation\Carbon\Traits\MagicParameter;
use Fy97Validation\Carbon\Traits\Mixin;
use Fy97Validation\Carbon\Traits\Options;
use Fy97Validation\Carbon\Traits\ToStringFormat;
use Closure;
use DateInterval;
 ...
 *
 * @method static Fy97Validation\CarbonInterval years($years = 1) Create instance specifying a number of years or modify the number of years if called on an instance.
 * @method static Fy97Validation\CarbonInterval year($years = 1) Alias for years()
 * @method static Fy97Validation\CarbonInterval months($months = 1) Create instance specifying a number of months or modify the number of months if called on an instance.
 * @method static Fy97Validation\CarbonInterval month($months = 1) Alias for months()
 * @method static Fy97Validation\CarbonInterval weeks($weeks = 1) Create instance specifying a number of weeks or modify the number of weeks if called on an instance.
 * @method static Fy97Validation\CarbonInterval week($weeks = 1) Alias for weeks()
 * @method static Fy97Validation\CarbonInterval days($days = 1) Create instance specifying a number of days or modify the number of days if called on an instance.
 * @method static Fy97Validation\CarbonInterval dayz($days = 1) Alias for days()
 * @method static Fy97Validation\CarbonInterval daysExcludeWeeks($days = 1) Create instance specifying a number of days or modify the number of days (keeping the current number of weeks) if called on an instance.
 * @method static Fy97Validation\CarbonInterval dayzExcludeWeeks($days = 1) Alias for daysExcludeWeeks()
 * @method static Fy97Validation\CarbonInterval day($days = 1) Alias for days()
 * @method static Fy97Validation\CarbonInterval hours($hours = 1) Create instance specifying a number of hours or modify the number of hours if called on an instance.
 * @method static Fy97Validation\CarbonInterval hour($hours = 1) Alias for hours()
 * @method static Fy97Validation\CarbonInterval minutes($minutes = 1) Create instance specifying a number of minutes or modify the number of minutes if called on an instance.
 * @method static Fy97Validation\CarbonInterval minute($minutes = 1) Alias for minutes()
 * @method static Fy97Validation\CarbonInterval seconds($seconds = 1) Create instance specifying a number of seconds or modify the number of seconds if called on an instance.
 * @method static Fy97Validation\CarbonInterval second($seconds = 1) Alias for seconds()
 * @method static Fy97Validation\CarbonInterval milliseconds($milliseconds = 1) Create instance specifying a number of milliseconds or modify the number of milliseconds if called on an instance.
 * @method static Fy97Validation\CarbonInterval millisecond($milliseconds = 1) Alias for milliseconds()
 * @method static Fy97Validation\CarbonInterval microseconds($microseconds = 1) Create instance specifying a number of microseconds or modify the number of microseconds if called on an instance.
 * @method static Fy97Validation\CarbonInterval microsecond($microseconds = 1) Alias for microseconds()
 ...
 */
class Fy97Validation\CarbonInterval extends DateInterval implements Fy97Validation\CarbonConverterInterface
{
     ...
     *
     * @var \Fy97Validation\Symfony\Component\Translation\TranslatorInterface
     */
    ...
    /**
     * This option allow you to opt-in for the Fy97Validation\Carbon 3 behavior where float
     ...
     * code including the third-party dependencies that also may use Fy97Validation\Carbon will
    ...
    /**
     * Create a new Fy97Validation\CarbonInterval instance.
     *
    ...
    public function __construct($years = 1, $months = null, $weeks = null, $days = null, $hours = null, $minutes = null, $seconds = null, $microseconds = null)
    {
        ...
        if ($microseconds !== null) {
            $this->f = $microseconds / Fy97Validation\Carbon::MICROSECONDS_PER_SECOND;
        }
    }
     ...
     * else try to find the appropriate constant as the factor, such as Fy97Validation\Carbon::DAYS_PER_WEEK.
     *
     * @param string $source
     * @param string $target
     *
    ...
    public static function getFactorWithDefault($source, $target)
    {
        ...
        static $defaults = [
            'month' => ['year' => Fy97Validation\Carbon::MONTHS_PER_YEAR],
            'week' => ['month' => Fy97Validation\Carbon::WEEKS_PER_MONTH],
            'day' => ['week' => Fy97Validation\Carbon::DAYS_PER_WEEK],
            'hour' => ['day' => Fy97Validation\Carbon::HOURS_PER_DAY],
            'minute' => ['hour' => Fy97Validation\Carbon::MINUTES_PER_HOUR],
            'second' => ['minute' => Fy97Validation\Carbon::SECONDS_PER_MINUTE],
            'millisecond' => ['second' => Fy97Validation\Carbon::MILLISECONDS_PER_SECOND],
            'microsecond' => ['millisecond' => Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND],
        ];
    ...
    {
        return static::getFactor('dayz', 'weeks') ?: Fy97Validation\Carbon::DAYS_PER_WEEK;
    }
    ...
    {
        return static::getFactor('hours', 'dayz') ?: Fy97Validation\Carbon::HOURS_PER_DAY;
    }
    ...
    {
        return static::getFactor('minutes', 'hours') ?: Fy97Validation\Carbon::MINUTES_PER_HOUR;
    }
    ...
    {
        return static::getFactor('seconds', 'minutes') ?: Fy97Validation\Carbon::SECONDS_PER_MINUTE;
    }
    ...
    {
        return static::getFactor('milliseconds', 'seconds') ?: Fy97Validation\Carbon::MILLISECONDS_PER_SECOND;
    }
    ...
    {
        return static::getFactor('microseconds', 'milliseconds') ?: Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND;
    }

    /**
     * Create a new Fy97Validation\CarbonInterval instance from specific values.
     * This is an alias for the constructor that allows better fluent
     * syntax as it allows you to do Fy97Validation\CarbonInterval::create(1)->fn() rather than
     * (new Fy97Validation\CarbonInterval(1))->fn().
     *
     * @param int $years
     * @param int $months
     * @param int $weeks
    ...
    /**
     * Parse a string into a new Fy97Validation\CarbonInterval object according to the specified format.
     *
     * @example
     * ```
     * echo Fy97Validation\Carboninterval::createFromFormat('H:i', '1:30');
     * ```
     *
     ...
     *
     * @throws \Fy97Validation\Carbon\Exceptions\ParseErrorException when the $interval cannot be parsed as an interval.
     *
     * @return static
     */
    ...
    /**
     * Provide static helpers to create instances.  Allows Fy97Validation\CarbonInterval::years(3).
     *
    ...
    /**
     * Evaluate the PHP generated by var_export() and recreate the exported Fy97Validation\CarbonInterval instance.
     *
    ...
    /**
     * Creates a Fy97Validation\CarbonInterval from string.
     *
     * Format:
     *
    ...
    public static function fromString($intervalDefinition)
    {
        ...
        while ([$part, $value, $unit] = array_shift($parts)) {
            ...
            switch ($unit === 'µs' ? 'µs' : strtolower($unit)) {
                case 'millennia':
                case 'millennium':
                    $years += $intValue * Fy97Validation\CarbonInterface::YEARS_PER_MILLENNIUM;

                    break;

                case 'century':
                case 'centuries':
                    $years += $intValue * Fy97Validation\CarbonInterface::YEARS_PER_CENTURY;

                    break;

                case 'decade':
                case 'decades':
                    $years += $intValue * Fy97Validation\CarbonInterface::YEARS_PER_DECADE;
                ...
                case 'quarter':
                case 'quarters':
                    $months += $intValue * Fy97Validation\CarbonInterface::MONTHS_PER_QUARTER;
        ...
        return new static($years, $months, $weeks, $days, $hours, $minutes, $seconds, $milliseconds * Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND + $microseconds);
    }

    /**
     * Creates a Fy97Validation\CarbonInterval from string using a different locale.
     *
    ...
    /**
     * Create a Fy97Validation\CarbonInterval instance from a DateInterval one.  Can not instance
    ...
    /**
     * Make a Fy97Validation\CarbonInterval instance from given variable if possible.
     *
    ...
    /**
     * Get a part of the Fy97Validation\CarbonInterval object.
     *
     * @param string $name
     *
    ...
    public function get($name)
    {
        ...
        switch ($name) {
            ...
            case 'milli':
            case 'milliseconds':
                return (int) (round($this->f * Fy97Validation\Carbon::MICROSECONDS_PER_SECOND) / Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND);

            case 'micro':
            case 'microseconds':
                return (int) round($this->f * Fy97Validation\Carbon::MICROSECONDS_PER_SECOND);
    ...
    /**
     * Set a part of the Fy97Validation\CarbonInterval object.
     *
    ...
    public function set($name, $value = null)
    {
        ...
        foreach ($properties as $key => $value) {
            switch (Carbon::singularUnit(rtrim($key, 'z'))) {
                ...
                case 'milli':
                case 'millisecond':
                    $this->microseconds = $value * Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND + $this->microseconds % Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND;

                    break;

                case 'micro':
                case 'microsecond':
                    $this->f = $value / Fy97Validation\Carbon::MICROSECONDS_PER_SECOND;

                    break;

                default:
                    if ($this->localStrictModeEnabled ?? Fy97Validation\Carbon::isStrictModeEnabled()) {
     ...
     * @example
     * ```
     * Fy97Validation\CarbonInterval::macro('twice', function () {
     *   return $this->times(2);
     * });
     * echo Fy97Validation\CarbonInterval::hours(2)->twice();
     * ```
     *
     * @param string          $name
     ...
     * @example
     * ```
     * Fy97Validation\CarbonInterval::mixin(new class {
     ...
     *   }
     * });
     * echo Fy97Validation\CarbonInterval::hours(5)->hoursToDays() . "\n";
     * echo Fy97Validation\CarbonInterval::days(5)->daysToHours() . "\n";
     * ```
     *
    ...
    /**
     * Allow fluent calls on the setters... Fy97Validation\CarbonInterval::years(3)->months(5)->day().
     *
     * Note: This is done using the magic method to allow static and instance methods to
     *       have the same names.
     *
    ...
    public function __call($method, $parameters)
    {
        ...
        if (preg_match('/^(?<method>add|sub)(?<unit>[A-Z].*)$/', $method, $match)) {
            $value = $this->getMagicParameter($parameters, 0, Fy97Validation\Carbon::pluralUnit($match['unit']), 0);

            return $this->{$match['method']}($value, $match['unit']);
        }

        $value = $this->getMagicParameter($parameters, 0, Fy97Validation\Carbon::pluralUnit($method), 1);
    ...
    protected function getForHumansInitialVariables($syntax, $short)
    {
        ...
        if (\is_bool($syntax)) {
            return [
                'short' => $syntax,
                'syntax' => Fy97Validation\CarbonInterface::DIFF_ABSOLUTE,
            ];
        }
    ...
    {
        if ($options & Fy97Validation\CarbonInterface::ROUND) {
            return 'round';
        }

        if ($options & Fy97Validation\CarbonInterface::CEIL) {
            return 'ceil';
        }

        if ($options & Fy97Validation\CarbonInterface::FLOOR) {
            return 'floor';
        }
     ...
     * @example
     * ```
     * echo Fy97Validation\CarbonInterval::fromString('4d 3h 40m')->forHumans() . "\n";
     * echo Fy97Validation\CarbonInterval::fromString('4d 3h 40m')->forHumans(['parts' => 2]) . "\n";
     * echo Fy97Validation\CarbonInterval::fromString('4d 3h 40m')->forHumans(['parts' => 3, 'join' => true]) . "\n";
     * echo Fy97Validation\CarbonInterval::fromString('4d 3h 40m')->forHumans(['short' => true]) . "\n";
     * echo Fy97Validation\CarbonInterval::fromString('1d 24h')->forHumans(['join' => ' or ']) . "\n";
     * echo Fy97Validation\CarbonInterval::fromString('1d 24h')->forHumans(['minimumUnit' => 'hour']) . "\n";
     * ```
     *
     ...
     *                           Possible values:
     *                           - Fy97Validation\CarbonInterface::DIFF_ABSOLUTE          no modifiers
     *                           - Fy97Validation\CarbonInterface::DIFF_RELATIVE_TO_NOW   add ago/from now modifier
     *                           - Fy97Validation\CarbonInterface::DIFF_RELATIVE_TO_OTHER add before/after modifier
     *                           Default value: Fy97Validation\CarbonInterface::DIFF_ABSOLUTE
    ...
    public function forHumans($syntax = null, $short = false, $parts = -1, $options = null)
    {
        ...
        $syntax = (int) ($syntax ?? Fy97Validation\CarbonInterface::DIFF_ABSOLUTE);
        $absolute = $syntax === Fy97Validation\CarbonInterface::DIFF_ABSOLUTE;
        $relativeToNow = $syntax === Fy97Validation\CarbonInterface::DIFF_RELATIVE_TO_NOW;
        $count = 1;
        $unit = $short ? 's' : 'second';
        ...
        /** @var \Fy97Validation\Symfony\Component\Translation\Translator $translator */
        ...
        foreach ($diffIntervalArray as $diffIntervalData) {
            ...
            } elseif ($options & Fy97Validation\CarbonInterface::SEQUENTIAL_PARTS_ONLY && \count($interval) > 0) {
                break;
            }
        ...
        if (\count($interval) === 0) {
            if ($relativeToNow && $options & Fy97Validation\CarbonInterface::JUST_NOW) {
                $key = 'diff_now';
            ...
            $count = $options & Fy97Validation\CarbonInterface::NO_ZERO_DIFF ? 1 : 0;
        ...
        if ($parts === 1) {
            if ($relativeToNow && $unit === 'day') {
                if ($count === 1 && $options & Fy97Validation\CarbonInterface::ONE_DAY_WORDS) {
                ...
                if ($count === 2 && $options & Fy97Validation\CarbonInterface::TWO_DAY_WORDS) {
    ...
    public function toPeriod(...$params)
    {
        ...
        return Fy97Validation\CarbonPeriod::create($this, ...$params);
    }
    ...
    public static function getDateIntervalSpec(DateInterval $interval, bool $microseconds = false, array $skip = [])
    {
        ...
        if (
            $interval->days >= Fy97Validation\CarbonInterface::DAYS_PER_WEEK * Fy97Validation\CarbonInterface::WEEKS_PER_MONTH &&
    ...
    {
        $current = Fy97Validation\Carbon::now();
    ...
    public function total($unit)
    {
        ...
        $values = [
            ...
            'minutes' => $this->minutes,
            'seconds' => $this->seconds,
            'milliseconds' => (int) ($this->microseconds / Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND),
            'microseconds' => $this->microseconds % Fy97Validation\Carbon::MICROSECONDS_PER_MILLISECOND,
        ];
     ...
     *
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval
     *
     * @see equalTo()
     *
     * @return bool
     ...
     * @example
     * ```
     * Fy97Validation\CarbonInterval::hours(48)->between(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(3)); // true
     * Fy97Validation\CarbonInterval::hours(48)->between(CarbonInterval::day(), Fy97Validation\CarbonInterval::hours(36)); // false
     * Fy97Validation\CarbonInterval::hours(48)->between(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(2)); // true
     * Fy97Validation\CarbonInterval::hours(48)->between(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(2), false); // false
     * ```
     *
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval2
     ...
     * @example
     * ```
     * Fy97Validation\CarbonInterval::hours(48)->betweenExcluded(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(3)); // true
     * Fy97Validation\CarbonInterval::hours(48)->betweenExcluded(CarbonInterval::day(), Fy97Validation\CarbonInterval::hours(36)); // false
     * Fy97Validation\CarbonInterval::hours(48)->betweenExcluded(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(2)); // true
     * ```
     *
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval2
     ...
     * Fy97Validation\CarbonInterval::hours(48)->betweenExcluded(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(3)); // true
     * Fy97Validation\CarbonInterval::hours(48)->betweenExcluded(CarbonInterval::day(), Fy97Validation\CarbonInterval::hours(36)); // false
     * Fy97Validation\CarbonInterval::hours(48)->betweenExcluded(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(2)); // false
     * ```
     *
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval2
     ...
     * @example
     * ```
     * Fy97Validation\CarbonInterval::hours(48)->isBetween(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(3)); // true
     * Fy97Validation\CarbonInterval::hours(48)->isBetween(CarbonInterval::day(), Fy97Validation\CarbonInterval::hours(36)); // false
     * Fy97Validation\CarbonInterval::hours(48)->isBetween(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(2)); // true
     * Fy97Validation\CarbonInterval::hours(48)->isBetween(CarbonInterval::day(), Fy97Validation\CarbonInterval::days(2), false); // false
     * ```
     *
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval1
     * @param Fy97Validation\CarbonInterval|DateInterval|mixed $interval2
    ...
    public function roundUnit($unit, $precision = 1, $function = 'round')
    {
        ...
        $base = Fy97Validation\CarbonImmutable::parse('2000-01-01 00:00:00', 'UTC')
    ...
    private function checkIntegerValue(string $name, $value)
    {
        ...
        if (!self::$floatSettersEnabled) {
            ...
            @trigger_error(
                ...
                "- To adopt the new behavior globally, call Fy97Validation\CarbonInterval::enableFloatSetters()\n",
                \E_USER_DEPRECATED
            );
        }
    }
