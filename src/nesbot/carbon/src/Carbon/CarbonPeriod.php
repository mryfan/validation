...
use Fy\Carbon\Exceptions\InvalidCastException;
use Fy\Carbon\Exceptions\InvalidIntervalException;
use Fy\Carbon\Exceptions\InvalidPeriodDateException;
use Fy\Carbon\Exceptions\InvalidPeriodParameterException;
use Fy\Carbon\Exceptions\NotAFy\CarbonClassException;
use Fy\Carbon\Exceptions\NotAPeriodException;
use Fy\Carbon\Exceptions\UnknownGetterException;
use Fy\Carbon\Exceptions\UnknownMethodException;
use Fy\Carbon\Exceptions\UnreachableException;
use Fy\Carbon\Traits\IntervalRounding;
use Fy\Carbon\Traits\Mixin;
use Fy\Carbon\Traits\Options;
use Closure;
use Countable;
use DateInterval;
use DatePeriod;
 ...
 * @property-read Fy\CarbonInterface $start Period start date.
 * @property-read Fy\CarbonInterface $current Current date from the iteration.
 * @property-read Fy\CarbonInterface $end Period end date.
 * @property-read Fy\CarbonInterval $interval Underlying date interval instance. Always present, one day by default.
 *
 * @method static Fy\CarbonPeriod start($date, $inclusive = null) Create instance specifying start date or modify the start date if called on an instance.
 * @method static Fy\CarbonPeriod since($date, $inclusive = null) Alias for start().
 * @method static Fy\CarbonPeriod sinceNow($inclusive = null) Create instance with start date set to now or set the start date to now if called on an instance.
 * @method static Fy\CarbonPeriod end($date = null, $inclusive = null) Create instance specifying end date or modify the end date if called on an instance.
 * @method static Fy\CarbonPeriod until($date = null, $inclusive = null) Alias for end().
 * @method static Fy\CarbonPeriod untilNow($inclusive = null) Create instance with end date set to now or set the end date to now if called on an instance.
 * @method static Fy\CarbonPeriod dates($start, $end = null) Create instance with start and end dates or modify the start and end dates if called on an instance.
 * @method static Fy\CarbonPeriod between($start, $end = null) Create instance with start and end dates or modify the start and end dates if called on an instance.
 * @method static Fy\CarbonPeriod recurrences($recurrences = null) Create instance with maximum number of recurrences or modify the number of recurrences if called on an instance.
 * @method static Fy\CarbonPeriod times($recurrences = null) Alias for recurrences().
 * @method static Fy\CarbonPeriod options($options = null) Create instance with options or modify the options if called on an instance.
 * @method static Fy\CarbonPeriod toggle($options, $state = null) Create instance with options toggled on or off, or toggle options if called on an instance.
 * @method static Fy\CarbonPeriod filter($callback, $name = null) Create instance with filter added to the stack or append a filter if called on an instance.
 * @method static Fy\CarbonPeriod push($callback, $name = null) Alias for filter().
 * @method static Fy\CarbonPeriod prepend($callback, $name = null) Create instance with filter prepended to the stack or prepend a filter if called on an instance.
 * @method static Fy\CarbonPeriod filters(array $filters = []) Create instance with filters stack or replace the whole filters stack if called on an instance.
 * @method static Fy\CarbonPeriod interval($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy\CarbonPeriod each($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy\CarbonPeriod every($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy\CarbonPeriod step($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy\CarbonPeriod stepBy($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy\CarbonPeriod invert() Create instance with inverted date interval or invert the interval if called on an instance.
 * @method static Fy\CarbonPeriod years($years = 1) Create instance specifying a number of years for date interval or replace the interval by the given a number of years if called on an instance.
 * @method static Fy\CarbonPeriod year($years = 1) Alias for years().
 * @method static Fy\CarbonPeriod months($months = 1) Create instance specifying a number of months for date interval or replace the interval by the given a number of months if called on an instance.
 * @method static Fy\CarbonPeriod month($months = 1) Alias for months().
 * @method static Fy\CarbonPeriod weeks($weeks = 1) Create instance specifying a number of weeks for date interval or replace the interval by the given a number of weeks if called on an instance.
 * @method static Fy\CarbonPeriod week($weeks = 1) Alias for weeks().
 * @method static Fy\CarbonPeriod days($days = 1) Create instance specifying a number of days for date interval or replace the interval by the given a number of days if called on an instance.
 * @method static Fy\CarbonPeriod dayz($days = 1) Alias for days().
 * @method static Fy\CarbonPeriod day($days = 1) Alias for days().
 * @method static Fy\CarbonPeriod hours($hours = 1) Create instance specifying a number of hours for date interval or replace the interval by the given a number of hours if called on an instance.
 * @method static Fy\CarbonPeriod hour($hours = 1) Alias for hours().
 * @method static Fy\CarbonPeriod minutes($minutes = 1) Create instance specifying a number of minutes for date interval or replace the interval by the given a number of minutes if called on an instance.
 * @method static Fy\CarbonPeriod minute($minutes = 1) Alias for minutes().
 * @method static Fy\CarbonPeriod seconds($seconds = 1) Create instance specifying a number of seconds for date interval or replace the interval by the given a number of seconds if called on an instance.
 * @method static Fy\CarbonPeriod second($seconds = 1) Alias for seconds().
 ...
 */
class Fy\CarbonPeriod implements Iterator, Countable, JsonSerializable
{
    ...
    /**
     * Yield Fy\CarbonImmutable instances.
     *
     * @var int
     */
     ...
     * @var string
     */
    protected $dateClass = Fy\Carbon::class;

    /**
     * Underlying date interval instance. Always present, one day by default.
     *
     * @var Fy\CarbonInterval
     */
    ...
    /**
     * Make a Fy\CarbonPeriod instance from given variable if possible.
     *
     * @param mixed $var
     *
     * @return static|null
    ...
    /**
     * Create a new instance from a DatePeriod or Fy\CarbonPeriod object.
     *
     * @param Fy\CarbonPeriod|DatePeriod $period
     *
     * @return static
     */
    ...
    /**
     * Create Fy\CarbonPeriod from ISO 8601 string.
     *
     * @param string   $iso
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && $parsed = Fy\CarbonInterval::make($part)) {
                $interval = $part;
            } elseif ($start === null && $parsed = Fy\Carbon::make($part)) {
                $start = $part;
            } elseif ($end === null && $parsed = Fy\Carbon::make(static::addMissingParts($start ?? '', $part))) {
                $end = $part;
            } else {
     ...
     * @example
     * ```
     * Fy\CarbonPeriod::macro('middle', function () {
     *   return $this->getStartDate()->average($this->getEndDate());
     * });
     * echo Fy\CarbonPeriod::since('2011-05-12')->until('2011-06-03')->middle();
     * ```
     *
     * @param string          $name
     ...
     * @example
     * ```
     * Fy\CarbonPeriod::mixin(new class {
     ...
     *   }
     * });
     * echo Fy\CarbonPeriod::create('2000-01-01', '2000-02-01')->addDays(5)->subDays(3);
     * ```
     *
    ...
    public function __construct(...$arguments)
    {
        ...
        foreach ($arguments as $argument) {
            if ($this->dateInterval === null &&
                ...
                $parsed = @Fy\CarbonInterval::make($argument)
            ) {
                $this->setDateInterval($parsed);
            } elseif ($this->startDate === null && $parsed = Fy\Carbon::make($argument)) {
                $this->setStartDate($parsed);
            } elseif ($this->endDate === null && $parsed = Fy\Carbon::make($argument)) {
        ...
        if ($this->startDate === null) {
            $this->setStartDate(Fy\Carbon::now());
        }

        if ($this->dateInterval === null) {
            $this->setDateInterval(Fy\CarbonInterval::day());
     ...
     * @param string $name
     *
     * @return bool|Fy\CarbonInterface|Fy\CarbonInterval|int|null
     */
    ...
    {
        if (!is_a($dateClass, Fy\CarbonInterface::class, true)) {
            throw new NotAFy\CarbonClassException($dateClass);
        }

        $this->dateClass = $dateClass;

        if (is_a($dateClass, Fy\Carbon::class, true)) {
            $this->toggleOptions(static::IMMUTABLE, false);
        } elseif (is_a($dateClass, Fy\CarbonImmutable::class, true)) {
    ...
    {
        if (!$interval = Fy\CarbonInterval::make($interval)) {
     ...
     * @param string|null $rounding Optional rounding 'floor', 'ceil', 'round' using the period interval.
     *
     * @return Fy\CarbonInterface|null
     */
     ...
     * @example
     * ```
     * var_dump(Fy\CarbonPeriod::create('2021-01-05', '2021-02-15')->toDatePeriod());
     * ```
     *
     * @return DatePeriod
     */
     ...
     *
     * Fy\CarbonPeriod::days(3)->hours(5)->invert()
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new Fy\Carbon, $first);
            ...
            case 'untilNow':
                return $this->setEndDate(new Fy\Carbon, $first);
            ...
            case 'second':
                return $this->setDateInterval((
                    ...
                    [$this->isDefaultInterval ? new Fy\CarbonInterval('PT0S') : $this->dateInterval, $method]
                )(
                    \count($parameters) === 0 ? 1 : $first
                ));
        }

        if ($this->localStrictModeEnabled ?? Fy\Carbon::isStrictModeEnabled()) {
    ...
    private function getEndFromRecurrences()
    {
        ...
        if ($this->recurrences === INF) {
            ...
            return $start < $start->avoidMutation()->add($this->getDateInterval())
                ? Fy\CarbonImmutable::endOfTime()
                : Fy\CarbonImmutable::startOfTime();
        }
     ...
     *
     * @param Fy\CarbonPeriod|\DateTimeInterface|Fy\Carbon|Fy\CarbonImmutable|string $rangeOrRangeStart
     * @param \DateTimeInterface|Fy\Carbon|Fy\CarbonImmutable|string|null         $rangeEnd
     *
     * @return bool
     */
     ...
     * @example
     * ```
     * Fy\Carbon::create('2020-11-29')->daysUntil('2020-12-24')->forEach(function (Fy\Carbon $date) {
     ...
     * @example
     * ```
     * $period = Fy\Carbon::create('2020-11-29')->daysUntil('2020-12-24');
     * echo implode("\n", iterator_to_array($period->map(function (Fy\Carbon $date) {
    ...
    {
        return $this->getStartDate()->lessThan($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->getStartDate()->lessThanOrEqualTo($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->getStartDate()->greaterThan($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->getStartDate()->greaterThanOrEqualTo($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->getStartDate()->equalTo($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->calculateEnd()->lessThan($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->calculateEnd()->lessThanOrEqualTo($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->calculateEnd()->greaterThan($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->calculateEnd()->greaterThanOrEqualTo($this->resolveFy\Carbon($date));
    }
    ...
    {
        return $this->calculateEnd()->equalTo($this->resolveFy\Carbon($date));
    }
     ...
     *
     * @param \Fy\Carbon\Fy\Carbon|\Fy\Carbon\Fy\CarbonPeriod|\Fy\Carbon\Fy\CarbonInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return bool
     */
     ...
     *
     * @param \Fy\Carbon\Fy\CarbonPeriod|\DatePeriod|string $period
     *
     * @return bool
     */
    public function follows($period, ...$arguments): bool
    {
        $period = $this->resolveFy\CarbonPeriod($period, ...$arguments);
     ...
     *
     * Will create an automatic filter callback for one of Fy\Carbon's is* methods.
     *
     * @param array $parameters
     *
     * @return array
    ...
    protected function createFilterTuple(array $parameters)
    {
        ...
        if (!$this->isFy\CarbonPredicateMethod($method)) {
    ...
    /**
     * Return whether given callable is a string pointing to one of Fy\Carbon's is* methods
     ...
     * @return bool
     */
    protected function isFy\CarbonPredicateMethod($callable)
    {
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     *
     * @param \Fy\Carbon\Fy\Carbon $current
     * @param int            $key
     *
     * @return bool|string
     */
    ...
    {
        if (($this->getOptions() & static::IMMUTABLE) && $this->dateClass === Fy\Carbon::class) {
            $this->setDateClass(Fy\CarbonImmutable::class);
        } elseif (!($this->getOptions() & static::IMMUTABLE) && $this->dateClass === Fy\CarbonImmutable::class) {
            $this->setDateClass(Fy\Carbon::class);
        }
     ...
     *
     * @param Fy\CarbonInterface $date
     *
     * @return Fy\CarbonInterface
     */
    protected function prepareForReturn(Fy\CarbonInterface $date)
    {
    ...
    /**
     * Return the Fy\Carbon instance passed through, a now instance in the same timezone
     * if null given or parse the input if string given.
     *
     * @param \Fy\Carbon\Fy\Carbon|\Fy\Carbon\Fy\CarbonPeriod|\Fy\Carbon\Fy\CarbonInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return \Fy\Carbon\Fy\CarbonInterface
     */
    protected function resolveFy\Carbon($date = null)
    {
        return $this->getStartDate()->nowWithSameTz()->carbonize($date);
    }

    /**
     * Resolve passed arguments or DatePeriod to a Fy\CarbonPeriod object.
     *
     * @param mixed $period
     ...
     * @return static
     */
    protected function resolveFy\CarbonPeriod($period, ...$arguments)
    {
        if ($period instanceof self) {
            return $period;
        }
