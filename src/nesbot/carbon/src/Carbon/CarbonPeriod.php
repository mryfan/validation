...
use Fy97Validation\Carbon\Exceptions\EndLessPeriodException;
use Fy97Validation\Carbon\Exceptions\InvalidCastException;
use Fy97Validation\Carbon\Exceptions\InvalidIntervalException;
use Fy97Validation\Carbon\Exceptions\InvalidPeriodDateException;
use Fy97Validation\Carbon\Exceptions\InvalidPeriodParameterException;
use Fy97Validation\Carbon\Exceptions\NotACarbonClassException;
use Fy97Validation\Carbon\Exceptions\NotAPeriodException;
use Fy97Validation\Carbon\Exceptions\UnknownGetterException;
use Fy97Validation\Carbon\Exceptions\UnknownMethodException;
use Fy97Validation\Carbon\Exceptions\UnreachableException;
 ...
 * @property-read Fy97Validation\CarbonInterface $start Period start date.
 * @property-read Fy97Validation\CarbonInterface $current Current date from the iteration.
 * @property-read Fy97Validation\CarbonInterface $end Period end date.
 * @property-read Fy97Validation\CarbonInterval $interval Underlying date interval instance. Always present, one day by default.
 *
 * @method static Fy97Validation\CarbonPeriod start($date, $inclusive = null) Create instance specifying start date or modify the start date if called on an instance.
 * @method static Fy97Validation\CarbonPeriod since($date, $inclusive = null) Alias for start().
 * @method static Fy97Validation\CarbonPeriod sinceNow($inclusive = null) Create instance with start date set to now or set the start date to now if called on an instance.
 * @method static Fy97Validation\CarbonPeriod end($date = null, $inclusive = null) Create instance specifying end date or modify the end date if called on an instance.
 * @method static Fy97Validation\CarbonPeriod until($date = null, $inclusive = null) Alias for end().
 * @method static Fy97Validation\CarbonPeriod untilNow($inclusive = null) Create instance with end date set to now or set the end date to now if called on an instance.
 * @method static Fy97Validation\CarbonPeriod dates($start, $end = null) Create instance with start and end dates or modify the start and end dates if called on an instance.
 * @method static Fy97Validation\CarbonPeriod between($start, $end = null) Create instance with start and end dates or modify the start and end dates if called on an instance.
 * @method static Fy97Validation\CarbonPeriod recurrences($recurrences = null) Create instance with maximum number of recurrences or modify the number of recurrences if called on an instance.
 * @method static Fy97Validation\CarbonPeriod times($recurrences = null) Alias for recurrences().
 * @method static Fy97Validation\CarbonPeriod options($options = null) Create instance with options or modify the options if called on an instance.
 * @method static Fy97Validation\CarbonPeriod toggle($options, $state = null) Create instance with options toggled on or off, or toggle options if called on an instance.
 * @method static Fy97Validation\CarbonPeriod filter($callback, $name = null) Create instance with filter added to the stack or append a filter if called on an instance.
 * @method static Fy97Validation\CarbonPeriod push($callback, $name = null) Alias for filter().
 * @method static Fy97Validation\CarbonPeriod prepend($callback, $name = null) Create instance with filter prepended to the stack or prepend a filter if called on an instance.
 * @method static Fy97Validation\CarbonPeriod filters(array $filters = []) Create instance with filters stack or replace the whole filters stack if called on an instance.
 * @method static Fy97Validation\CarbonPeriod interval($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy97Validation\CarbonPeriod each($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy97Validation\CarbonPeriod every($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy97Validation\CarbonPeriod step($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy97Validation\CarbonPeriod stepBy($interval) Create instance with given date interval or modify the interval if called on an instance.
 * @method static Fy97Validation\CarbonPeriod invert() Create instance with inverted date interval or invert the interval if called on an instance.
 * @method static Fy97Validation\CarbonPeriod years($years = 1) Create instance specifying a number of years for date interval or replace the interval by the given a number of years if called on an instance.
 * @method static Fy97Validation\CarbonPeriod year($years = 1) Alias for years().
 * @method static Fy97Validation\CarbonPeriod months($months = 1) Create instance specifying a number of months for date interval or replace the interval by the given a number of months if called on an instance.
 * @method static Fy97Validation\CarbonPeriod month($months = 1) Alias for months().
 * @method static Fy97Validation\CarbonPeriod weeks($weeks = 1) Create instance specifying a number of weeks for date interval or replace the interval by the given a number of weeks if called on an instance.
 * @method static Fy97Validation\CarbonPeriod week($weeks = 1) Alias for weeks().
 * @method static Fy97Validation\CarbonPeriod days($days = 1) Create instance specifying a number of days for date interval or replace the interval by the given a number of days if called on an instance.
 * @method static Fy97Validation\CarbonPeriod dayz($days = 1) Alias for days().
 * @method static Fy97Validation\CarbonPeriod day($days = 1) Alias for days().
 * @method static Fy97Validation\CarbonPeriod hours($hours = 1) Create instance specifying a number of hours for date interval or replace the interval by the given a number of hours if called on an instance.
 * @method static Fy97Validation\CarbonPeriod hour($hours = 1) Alias for hours().
 * @method static Fy97Validation\CarbonPeriod minutes($minutes = 1) Create instance specifying a number of minutes for date interval or replace the interval by the given a number of minutes if called on an instance.
 * @method static Fy97Validation\CarbonPeriod minute($minutes = 1) Alias for minutes().
 * @method static Fy97Validation\CarbonPeriod seconds($seconds = 1) Create instance specifying a number of seconds for date interval or replace the interval by the given a number of seconds if called on an instance.
 * @method static Fy97Validation\CarbonPeriod second($seconds = 1) Alias for seconds().
 ...
 */
class Fy97Validation\CarbonPeriod implements Iterator, Countable, JsonSerializable
{
    ...
    /**
     * Yield Fy97Validation\CarbonImmutable instances.
     *
     * @var int
     */
     ...
     * @var string
     */
    protected $dateClass = Fy97Validation\Carbon::class;

    /**
     * Underlying date interval instance. Always present, one day by default.
     *
     * @var Fy97Validation\CarbonInterval
     */
     ...
     *
     * @var Fy97Validation\CarbonInterface
     */
    protected $startDate;
     ...
     *
     * @var Fy97Validation\CarbonInterface|null
     */
    protected $endDate;
    ...
    /**
     * Make a Fy97Validation\CarbonPeriod instance from given variable if possible.
     *
     * @param mixed $var
     *
     * @return static|null
    ...
    /**
     * Create a new instance from a DatePeriod or Fy97Validation\CarbonPeriod object.
     *
     * @param Fy97Validation\CarbonPeriod|DatePeriod $period
     *
     * @return static
     */
    ...
    /**
     * Create Fy97Validation\CarbonPeriod from ISO 8601 string.
     *
     * @param string   $iso
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && $parsed = Fy97Validation\CarbonInterval::make($part)) {
                $interval = $part;
            } elseif ($start === null && $parsed = Fy97Validation\Carbon::make($part)) {
                $start = $part;
            } elseif ($end === null && $parsed = Fy97Validation\Carbon::make(static::addMissingParts($start ?? '', $part))) {
                $end = $part;
            } else {
     ...
     * @example
     * ```
     * Fy97Validation\CarbonPeriod::macro('middle', function () {
     *   return $this->getStartDate()->average($this->getEndDate());
     * });
     * echo Fy97Validation\CarbonPeriod::since('2011-05-12')->until('2011-06-03')->middle();
     * ```
     *
     * @param string          $name
     ...
     * @example
     * ```
     * Fy97Validation\CarbonPeriod::mixin(new class {
     ...
     *   }
     * });
     * echo Fy97Validation\CarbonPeriod::create('2000-01-01', '2000-02-01')->addDays(5)->subDays(3);
     * ```
     *
    ...
    /**
     * Fy97Validation\CarbonPeriod constructor.
     *
     * @SuppressWarnings(PHPMD.ElseExpression)
     *
    ...
    {
        if (!is_a($dateClass, Fy97Validation\CarbonInterface::class, true)) {
        ...
        if (is_a($dateClass, Fy97Validation\Carbon::class, true)) {
            $self->options = $self->options & ~static::IMMUTABLE;
        } elseif (is_a($dateClass, Fy97Validation\CarbonImmutable::class, true)) {
            $self->options = $self->options | static::IMMUTABLE;
        }
    ...
    {
        if (!$interval = Fy97Validation\CarbonInterval::make($interval)) {
     ...
     *
     * @return Fy97Validation\CarbonInterval
     */
     ...
     * @param string|null $rounding Optional rounding 'floor', 'ceil', 'round' using the period interval.
     *
     * @return Fy97Validation\CarbonInterface|null
     */
     ...
     *
     * Fy97Validation\CarbonPeriod::days(3)->hours(5)->invert()
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new Fy97Validation\Carbon(), ...$parameters);
            ...
            case 'untilNow':
                return $this->setEndDate(new Fy97Validation\Carbon(), ...$parameters);
            ...
            case 'second':
                return $this->setDateInterval((
                    ...
                    [$this->isDefaultInterval ? new Fy97Validation\CarbonInterval('PT0S') : $this->dateInterval, $method]
                )(...$parameters));
        }
    ...
    private function getEndFromRecurrences()
    {
        ...
        if ($this->recurrences === INF) {
            ...
            return $start < $start->avoidMutation()->add($this->getDateInterval())
                ? Fy97Validation\CarbonImmutable::endOfTime()
                : Fy97Validation\CarbonImmutable::startOfTime();
        }
     ...
     *
     * @param Fy97Validation\CarbonPeriod|\DateTimeInterface|Carbon|CarbonImmutable|string $rangeOrRangeStart
     ...
     * @example
     * ```
     * Fy97Validation\Carbon::create('2020-11-29')->daysUntil('2020-12-24')->forEach(function (Carbon $date) {
     ...
     * @example
     * ```
     * $period = Fy97Validation\Carbon::create('2020-11-29')->daysUntil('2020-12-24');
     ...
     *
     * @param \Fy97Validation\Carbon\Carbon|\Carbon\CarbonPeriod|\Carbon\CarbonInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return bool
     */
     ...
     *
     * @param \Fy97Validation\Carbon\CarbonPeriod|\DatePeriod|string $period
     *
     * @return bool
     */
     ...
     *
     * Will create an automatic filter callback for one of Fy97Validation\Carbon's is* methods.
     *
     * @param array $parameters
     *
     * @return array
    ...
    /**
     * Return whether given callable is a string pointing to one of Fy97Validation\Carbon's is* methods
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     *
     * @param \Fy97Validation\Carbon\Carbon $current
     * @param int            $key
     *
     * @return bool|string
     */
    ...
    {
        if (($this->getOptions() & static::IMMUTABLE) && $this->dateClass === Fy97Validation\Carbon::class) {
            $this->dateClass = Fy97Validation\CarbonImmutable::class;
        } elseif (!($this->getOptions() & static::IMMUTABLE) && $this->dateClass === Fy97Validation\CarbonImmutable::class) {
            $this->dateClass = Fy97Validation\Carbon::class;
        }
     ...
     *
     * @param Fy97Validation\CarbonInterface $date
     *
     * @return Fy97Validation\CarbonInterface
     */
    ...
    /**
     * Return the Fy97Validation\Carbon instance passed through, a now instance in the same timezone
     * if null given or parse the input if string given.
     *
     * @param \Fy97Validation\Carbon\Carbon|\Carbon\CarbonPeriod|\Carbon\CarbonInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return \Fy97Validation\Carbon\CarbonInterface
     */
    ...
    /**
     * Resolve passed arguments or DatePeriod to a Fy97Validation\CarbonPeriod object.
     *
     * @param mixed $period
    ...
    private function makeDateTime($value): ?DateTimeInterface
    {
        ...
        if (\is_string($value)) {
            ...
            if (!preg_match('/^P[\dT]/', $value) &&
                ...
                return Fy97Validation\Carbon::parse($value, $this->tzName);
            }
        }
    ...
    {
        return $date instanceof Fy97Validation\CarbonInterface && ($date->isEndOfTime() || $date->isStartOfTime());
    }
    ...
    private function rawDate($date): ?DateTimeInterface
    {
        ...
        if ($date instanceof Fy97Validation\CarbonInterface) {
