...
function getOpenCollectiveSponsors(): string
{
    ...
    $list = array_filter($members, static function ($member): bool {
        return ($member['lastTransactionAmount'] > 3 || $member['isActive']) &&
            ...
            (
                ...
                $member['lastTransactionAt'] > Fy97Validation\CarbonImmutable::now()
    ...
    $list = array_map(static function (array $member): array {
        $createdAt = Fy97Validation\CarbonImmutable::parse($member['createdAt']);
        $lastTransactionAt = Fy97Validation\CarbonImmutable::parse($member['lastTransactionAt']);
