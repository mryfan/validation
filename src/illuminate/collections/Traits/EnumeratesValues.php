...
use Fy97Validation\Illuminate\Support\Enumerable;
use Fy97Validation\Illuminate\Support\HigherOrderCollectionProxy;
...
trait EnumeratesValues
{
    ...
    public function value($key, $default = null)
    {
        if ($value = $this->firstWhere($key)) {
            return \Fy97Validation\Illuminate\Support\helpers_new::data_get($value, $key, $default);
        }
    ...
    {
        return $this->filter(
            fn ($item) => \Fy97Validation\Illuminate\Support\helpers_new::data_get($item, $key) < reset($values) || \Fy97Validation\Illuminate\Support\helpers_new::data_get($item, $key) > end($values)
        );
    }
        ...
        foreach ($this as $key => $value) {
            ...
            if (! is_array($result)) {
                throw new UnexpectedValueException(sprintf(
                    ...
                    \Fy97Validation\Illuminate\Support\helpers1_new::class_basename(static::class), gettype($result)
                ));
            }
        }
    ...
    public function __toString()
    {
        return $this->escapeWhenCastingToString
                    ? \Fy97Validation\Illuminate\Support\helpers1_new::e($this->toJson())
                    : $this->toJson();
    }
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use ($key, $operator, $value) {
            $retrieved = \Fy97Validation\Illuminate\Support\helpers_new::data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return fn ($item) => \Fy97Validation\Illuminate\Support\helpers_new::data_get($item, $value);
    }
