...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = \Fy97Validation\Illuminate\Support\helpers_new::collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = \Fy97Validation\Illuminate\Support\helpers_new::collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->mailables)->filter(function ($mailable) use ($type) {
    ...
    {
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }
