...
use Closure;
use Fy97Validation\Illuminate\Contracts\Mail\Factory;
use Fy97Validation\Illuminate\Contracts\Mail\Mailable;
use Fy97Validation\Illuminate\Contracts\Mail\Mailer;
use Fy97Validation\Illuminate\Contracts\Mail\MailQueue;
use Fy97Validation\Illuminate\Contracts\Queue\ShouldQueue;
use Illuminate\Mail\MailManager;
use Fy97Validation\Illuminate\Support\Traits\ForwardsCalls;
...
class MailFake implements Factory, Fake, Mailer, MailQueue
{
    ...
    {
        $mailableNames = \Fy97Validation\Illuminate\Support\helpers_new::collect($this->mailables)->map(
    ...
    {
        $mailableNames = \Fy97Validation\Illuminate\Support\helpers_new::collect($this->queuedMailables)->map(
            fn ($mailable) => get_class($mailable)
        )->join(', ');
    ...
    {
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->mailables)->filter(fn ($mailable) => $mailable instanceof $type);
    }
    ...
    {
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->queuedMailables)->filter(fn ($mailable) => $mailable instanceof $type);
    }
     ...
     *
     * @param  \Fy97Validation\Illuminate\Contracts\Mail\Mailable|string|array  $view
     * @param  array  $data
