...
use Fy97Validation\Illuminate\Contracts\Bus\QueueingDispatcher;
use Fy97Validation\Illuminate\Support\Arr;
use Fy97Validation\Illuminate\Support\Collection;
use Fy97Validation\Illuminate\Support\Traits\ReflectsClosures;
...
class BusFake implements Fake, QueueingDispatcher
{
     ...
     *
     * @var \Fy97Validation\Illuminate\Contracts\Bus\QueueingDispatcher
     */
    public $dispatcher;
     ...
     *
     * @param  \Fy97Validation\Illuminate\Contracts\Bus\QueueingDispatcher  $dispatcher
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            \Fy97Validation\Illuminate\Support\helpers_new::collect($expectedChain)->isNotEmpty(),
    ...
    {
        return \Fy97Validation\Illuminate\Support\helpers1_new::tap(clone $job, function ($job) {
    ...
    {
        $chain = \Fy97Validation\Illuminate\Support\helpers_new::collect($expectedChain)->map(fn ($job) => serialize($job))->all();
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return \Fy97Validation\Illuminate\Support\helpers_new::collect($chain)->map(
    ...
    {
        return ! \Fy97Validation\Illuminate\Support\helpers_new::collect($chain)->contains(fn ($job) => ! is_object($job));
    }
     ...
     * @param  string  $command
     * @param  callable|null  $callback
     * @return \Fy97Validation\Illuminate\Support\Collection
     */
    public function dispatched($command, $callback = null)
    {
        if (! $this->hasDispatched($command)) {
            return \Fy97Validation\Illuminate\Support\helpers_new::collect();
        }

        $callback = $callback ?: fn () => true;

        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->commands[$command])->filter(fn ($command) => $callback($command));
    }
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->commandsSync[$command])->filter(fn ($command) => $callback($command));
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->commandsAfterResponse[$command])->filter(fn ($command) => $callback($command));
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->batches)->filter(fn ($batch) => $callback($batch));
    }
     ...
     *
     * @param  \Fy97Validation\Illuminate\Support\Collection|array  $jobs
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->jobsToFake)
    ...
    {
        return \Fy97Validation\Illuminate\Support\helpers_new::collect($this->jobsToDispatch)
            ->filter(function ($job) use ($command) {
                return $job instanceof Closure
                    ? $job($command)
                    : $job === get_class($command);
