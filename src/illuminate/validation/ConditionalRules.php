...
class ConditionalRules
{
    ...
    public function rules(array $data = [])
    {
        return is_string($this->rules)
                    ? explode('|', $this->rules)
                    : \Fy97Validation\Illuminate\Support\helpers_new::value($this->rules, new Fluent($data));
    }
    ...
    public function defaultRules(array $data = [])
    {
        return is_string($this->defaultRules)
                    ...
                    : \Fy97Validation\Illuminate\Support\helpers_new::value($this->defaultRules, new Fluent($data));
    }
}
