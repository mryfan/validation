...
use Fy97Validation\Symfony\Component\HttpFoundation\Exception\ConflictingHeadersException;
use Fy97Validation\Symfony\Component\HttpFoundation\Exception\JsonException;
use Fy97Validation\Symfony\Component\HttpFoundation\Exception\SessionNotFoundException;
use Fy97Validation\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException;
use Fy97Validation\Symfony\Component\HttpFoundation\Session\SessionInterface;
...
class Request
{
    ...
    public static function setTrustedProxies(array $proxies, int $trustedHeaderSet)
    {
        if (self::HEADER_X_FORWARDED_ALL === $trustedHeaderSet) {
            \Fy97Validation\Fy97Validation/symfony/deprecation-contractfunction_new::trigger_deprecation('symfony/http-foundation', '5.2', 'The "HEADER_X_FORWARDED_ALL" constant is deprecated, use either "HEADER_X_FORWARDED_FOR | HEADER_X_FORWARDED_HOST | HEADER_X_FORWARDED_PORT | HEADER_X_FORWARDED_PROTO" or "HEADER_X_FORWARDED_AWS_ELB" or "HEADER_X_FORWARDED_TRAEFIK" constants instead.');
        }
    ...
    public function toArray()
    {
        ...
        if (!\is_array($content)) {
            throw new JsonException(sprintf('JSON content was expected to decode to an array, "%s" returned.', \Fy97Validation\Symfony\Polyfill\Php80\bootstrap_new::get_debug_type($content)));
        }
    ...
    private static function createRequestFromFactory(array $query = [], array $request = [], array $attributes = [], array $cookies = [], array $files = [], array $server = [], $content = null): self
    {
        if (self::$requestFactory) {
            ...
            if (!$request instanceof self) {
                throw new \LogicException('The Request factory must return an instance of Fy97Validation\Symfony\Component\HttpFoundation\Request.');
            }
