        ...
        foreach ($options as $key => $value) {
            if (isset($validOptions[$key])) {
                if (str_starts_with($key, 'upload_progress.')) {
                    \Fy97Validation\deprecation_contracts\function_new::trigger_deprecation('symfony/http-foundation', '5.4', 'Support for the "%s" session option is deprecated. The settings prefixed with "session.upload_progress." can not be changed at runtime.', $key);
                    continue;
                }
                if ('url_rewriter.tags' === $key) {
                    \Fy97Validation\deprecation_contracts\function_new::trigger_deprecation('symfony/http-foundation', '5.4', 'Support for the "%s" session option is deprecated. Use "trans_sid_tags" instead.', $key);
                }
