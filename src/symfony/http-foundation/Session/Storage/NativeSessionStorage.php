...
use Fy97Validation\Symfony\Component\HttpFoundation\Session\SessionBagInterface;
use Fy97Validation\Symfony\Component\HttpFoundation\Session\SessionUtils;
use Fy97Validation\Symfony\Component\HttpFoundation\Session\Storage\Handler\StrictSessionHandler;
use Fy97Validation\Symfony\Component\HttpFoundation\Session\Storage\Proxy\AbstractProxy;
use Fy97Validation\Symfony\Component\HttpFoundation\Session\Storage\Proxy\SessionHandlerProxy;
...
class NativeSessionStorage implements SessionStorageInterface
{
    ...
    public function save()
    {
        ...
        $previousHandler = set_error_handler(function ($type, $msg, $file, $line) use (&$previousHandler) {
            if (\E_WARNING === $type && \Fy97Validation\Symfony\Polyfill\Php80\bootstrap_new::str_starts_with($msg, 'session_write_close():')) {
        ...
        foreach ($options as $key => $value) {
            if (isset($validOptions[$key])) {
                if (str_starts_with($key, 'upload_progress.')) {
                    \Fy97Validation\Fy97Validation/symfony/deprecation-contractfunction_new::trigger_deprecation('symfony/http-foundation', '5.4', 'Support for the "%s" session option is deprecated. The settings prefixed with "session.upload_progress." can not be changed at runtime.', $key);
                    continue;
                }
                if ('url_rewriter.tags' === $key) {
                    \Fy97Validation\Fy97Validation/symfony/deprecation-contractfunction_new::trigger_deprecation('symfony/http-foundation', '5.4', 'Support for the "%s" session option is deprecated. Use "trans_sid_tags" instead.', $key);
                }
