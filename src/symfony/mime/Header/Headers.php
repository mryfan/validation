...
namespace Fy97Validation\Symfony\Component\Mime\Header;

use Fy97Validation\Symfony\Component\Mime\Address;
...
final class Headers
{
    ...
    public static function checkHeaderClass(HeaderInterface $header): void
    {
        ...
        if (($c = self::HEADER_CLASS_MAP[$name] ?? null) && !$header instanceof $c) {
            throw new LogicException(sprintf('The "%s" header must be an instance of "%s" (got "%s").', $header->getName(), $c, \Fy97Validation\Symfony\Polyfill\Php80\bootstrap_new::get_debug_type($header)));
        }
    }
